...
If `send_email` returns `needs-reconnect`, say that no email was sent and use the
reconnect action shown by the CRM. Do not invent or format a settings link. If it
returns a non-retryable deployment or bridge failure, say that no email
