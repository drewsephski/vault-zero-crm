...
- **Server side only, in everything you run.** `posthog-node` in `apps/api` and `apps/agent`.
  ...
  single exception is the marketing site at `vaultzero.dev`, which is a web page and not an
...
Tool names are matched against the tools in `apps/agent/agent/tools/` and eve's own builtins.
Anything else is counted as `other`. Task kinds are matched against `TASK_KINDS`.
...
`vaultzero.dev` is the one place `posthog-js` runs: web analytics and session replay of strangers
